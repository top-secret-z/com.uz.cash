...
define(["require", "exports", "tslib", "WoltLabSuite/Core/Ajax", "WoltLabSuite/Core/Language", "WoltLabSuite/Core/Ui/Dialog", "WoltLabSuite/Core/Ui/Notification", "WoltLabSuite/Core/Dom/Util", "WoltLabSuite/Core/Dom/Traverse"], function (require, exports, tslib_1, Ajax, Language, Dialog_1, UiNotification, Util_1, DomTraverse) {
    ...
    class UZCashUserClaimEdit {
        constructor() {
            var buttons = document.querySelectorAll('.jsClaimEditButton');
        ...
        _submit() {
            ...
            if(isNaN(amount) || amount <= 0) {
                if (!amountError) {
                    ...
                    amountError.innerText = Language.get('cash.claim.user.amount.error');
            ...
            if (subject.length > 255) {
                if (!subjectError) {
                    ...
                    subjectError.innerText = Language.get('cash.claim.user.subject.error');
                    Util_1.default.insertAfter(subjectError, subjectInput);
                    subjectInput.closest('dl').classList.add('formError');
                }
                else {
                    subjectError.innerText = Language.get('cash.claim.user.amount.error');
                }
                return;
            }
            else {
            ...
            Ajax.api(this, {
                actionName:    'saveClaimEditDialog',
                parameters:    {
                    amount:        amount,
                    currency:    currency,
                    subject:    subject,
        ...
        _ajaxSuccess(data) {
            switch (data.actionName) {
                case 'getClaimEditDialog':
                    this._render(data);
                    break;
                case 'saveClaimEditDialog':
                    UiNotification.show(Language.get('cash.claim.user.success'));
                    Dialog_1.default.close(this);
        ...
        _dialogSetup() {
            return {
                id: 'getUserClaimEditDialog',
                options:     { 
                    title: Language.get('cash.claim.user.edit') 
                },
                source: null
            };
        }
    ...
    let uZCashUserClaimEdit;
    function init() {
        if (!uZCashUserClaimEdit) {
            uZCashUserClaimEdit = new UZCashUserClaimEdit();
        }
    }
    exports.init = init;
});
