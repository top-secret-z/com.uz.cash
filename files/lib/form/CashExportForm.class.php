...
class CashExportForm extends AbstractForm {
	...
	public function readFormParameters() {
		...
		if (isset($_POST['openClaims'])) $this->openClaims = intval($_POST['openClaims']);
		if (isset($_POST['startDate'])) $this->startDate = $_POST['startDate'];
		if (isset($_POST['endDate'])) $this->endDate = $_POST['endDate'];
		
		if (isset($_POST['selectedCurrencies'])) $this->selectedCurrencies = array_keys(ArrayUtil::trim($_POST['selectedCurrencies']));
		if (isset($_POST['selectedContents'])) $this->selectedContents = array_keys(ArrayUtil::trim($_POST['selectedContents']));
	...
	public function save() {
		...
		foreach ($cashs as $cash) {
			// check open claims
			if ($this->openClaims && $cash->type == 'claimSent') continue;
			...
			if ($includePostings && $cash->type == 'posting') {
				if (!isset($postingCategory[$cash->typeID])) {
					...
					if (!$posting->postingID) continue;
					$postingCategory[$cash->typeID] = $posting->categoryID;
				}
				if (!in_array($postingCategory[$cash->typeID], $this->categoryIDs)) continue;
			...
			if ($includeClaims && strrpos($cash->type, 'claim') !== false) {
				...
				if (!in_array($claimCategory[$userClaim->claimID], $this->categoryIDs)) continue;
			...
			if ($includeCredits && strrpos($cash->type, 'credit') !== false) {
				...
				if (!isset($creditCategory[$userCredit->creditID])) {
					...
					if (!$credit->creditID) continue;
					$creditCategory[$userCredit->creditID] = $credit->categoryID;
				}
				if (!in_array($creditCategory[$userCredit->creditID], $this->categoryIDs)) continue;
			...
			$amount = number_format(round($cash->amount, 2), 2, WCF::getLanguage()->get('wcf.global.decimalPoint'), WCF::getLanguage()->get('wcf.global.thousandsSeparator'));
