...
class PostingListPage extends SortablePage
{
    ...
    public function readParameters()
    {
        ...
        if (!empty($this->categoryID)) {
            $postingList->getConditionBuilder()->add('categoryID = ?', [$this->categoryID]);
        }
        if (!empty($this->currency)) {
            $postingList->getConditionBuilder()->add('currency LIKE ?', [$this->currency]);
        }
        if (!empty($this->subject)) {
            $postingList->getConditionBuilder()->add('subject LIKE ?', ['%' . $this->subject . '%']);
        }
        if (!empty($this->type)) {
            $postingList->getConditionBuilder()->add('type LIKE ?', [$this->type]);
        }
        ...
        if (\count($postings)) {
            foreach ($postings as $posting) {
                ...
                if (isset($this->balance[$posting->currency])) {
                    $this->balance[$posting->currency] += $amount;
                } else {
                    $this->balance[$posting->currency] = $amount;
                }
            }
    ...
    public function assignVariables()
    {
        ...
        if (\count($this->balance)) {
            foreach ($this->balance as $currency => $amount) {
                $amount = \number_format(\round($amount, 2), 2, WCF::getLanguage()->get('wcf.global.decimalPoint'), WCF::getLanguage()->get('wcf.global.thousandsSeparator'));
